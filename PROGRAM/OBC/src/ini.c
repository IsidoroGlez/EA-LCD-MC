  ...
  for(ibit=0;ibit<nbits;ibit++){
	...
	for(x=0;x<Lx;x++){
	  ...
	  if(z<(Lz-1))
	    ...
	    Jz[ibit][site]=Jy[ibit][site];
	  if(z==0)
	    J_falso_z0[site]=Jy[ibit][site];
	...
	parity=(x^y^z)&1;
	if(parity==0){
	  J_falso_z0[site]=Jy[ibit][site]^1;
	}	
	site++;
      }
    }
    ...
    //Everything generated. We need to simetrize the OBC conditions.
    z=0;
    site=0;
    for(y=0;y<Ly;y++){
      neigh_my=y_m[y];
	...
	parity=(x^y^z)&1;
	if(parity){
	  J_falso_z0[site]=J_falso_z0[site+neigh_my];
	}	
	site++;
      }
    }
	...
	parity=(x^y^z)&1;
	if(parity){
	  Jz[ibit][site]=Jz[ibit][site+neigh_my];
	}	
	site++;
      }
    }

    //Forward couplings correctly simetrize. We need to do the same with the backward
    ...
    for(z=0;z<Lz;z++){
      ...
      for(y=0;y<Ly;y++){
        ...
        for(x=0;x<Lx;x++){
          ...
          Jmx[ibit][site] = Jx[ibit][site+neigh_mx];
          Jmy[ibit][site] = Jy[ibit][site+neigh_my];

	  if(z>0)
	    Jmz[ibit][site] = Jz[ibit][site+neigh_mz];
	  else
	    Jmz[ibit][site] = J_falso_z0[site];
...
void packing_J(int nbits)
{
  ...
  for(ibit=0;ibit<nbits;ibit++){
	...
	for(x=0;x<L;x++){
	  ...
	  J[pointer+DIM] |= ((Jmx[ibit][site])&1ULL)<<bit;
	  //	  J[DEGREE*punto_n+DIM] |= ((Jmx[ibit][site+neigh_px])&1ULL)<<bit;

	  pointer++;
	  punto_n = MSC_neigh[pointer];
	  J[pointer] |= ((Jy[ibit][site])&1ULL)<<bit;
	  J[pointer+DIM] |= ((Jmy[ibit][site])&1ULL)<<bit;
	  //	  J[DEGREE*punto_n+1+DIM] |= ((Jmy[ibit][site+neigh_py])&1ULL)<<bit;

	  pointer++;
	  punto_n = MSC_neigh[pointer];
	  J[pointer] |= ((Jz[ibit][site])&1ULL)<<bit;
	  J[pointer+DIM] |= ((Jmz[ibit][site])&1ULL)<<bit;
	  //	  J[DEGREE*punto_n+2+DIM] |= ((Jmz[ibit][site+neigh_pz])&1ULL)<<bit;
