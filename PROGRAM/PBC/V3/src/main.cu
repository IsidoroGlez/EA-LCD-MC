...
int main(int argc, char **argv){
  ...
  int numbins=data.nbin;
  
  stop(&numbins); // Last chance for changing nbin
  ...
  for(ibin = data.itcut; ibin < numbins; ibin++){
    ...
    stop(&numbins); // para o alarga el loop
...
#define BETATHRESHOLD 0.5
  ...
  unsigned int OverlapnumBlocks=NBETAS*(( (HALF_MSC_VNRNBETAS/NR)+numThreadsPerBlock-1 )/numThreadsPerBlock);  
