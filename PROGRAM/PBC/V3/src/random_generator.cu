...

{
  ...
  memset(&salida, 0, sizeof(s_aleatorio_HQ_64bits));
...
void generate_seeds_from_one(randint *seed, randint *list_of_seeds, int nseeds)
{

  uint64_t temporal;
  uint32_t bobo;
  volatile int iseed, paranoia,i;
  ...
  *seed=comprueba_semilla(*seed);
  ...
  for(iseed=0;iseed<nseeds;iseed++){
    ...
    temporal>>=62;//2 most significant bits (prima erano 3...)
